...
static void atsmb_set_ios(struct mmc_host *mmc, struct mmc_ios *ios)
{
	...
	if (ios->power_mode == MMC_POWER_OFF) {
		if (MMC0_DRIVER_VERSION == MMC_DRV_3445) {
            /* stop SD output clock */
			*ATSMB0_BUS_MODE &= ~(ATSMB_CST);

            /*  disable SD Card power  */			
			/*set SD0 power pin as GPO pin*/
            GPIO_CTRL_GP18_SD0PWRSW_BYTE_VAL |= GPIO_SD0_POWER;
            GPIO_OC_GP18_SD0PWRSW_BYTE_VAL |= GPIO_SD0_POWER;
           
            
			/*set internal pull up*/
            GPIO_PULL_CTRL_GP18_SD0PWRSW_BYTE_VAL |= GPIO_SD0_POWER;
			/*set internal pull enable*/
            GPIO_PULL_EN_GP18_SD0PWRSW_BYTE_VAL |= GPIO_SD0_POWER;
			/*disable SD0 power*/
			GPIO_OD_GP18_SD0PWRSW_BYTE_VAL |= GPIO_SD0_POWER;
                    
			/* Disable Pull up/down resister of SD Bus */
			GPIO_PULL_CTRL_GP13_XDIO_BYTE_VAL &=  ~SD0_PIN;

			/*  Config SD0 to GPIO  */
            GPIO_CTRL_GP14_SD_BYTE_VAL |= SD0_PIN;
			
			/*  SD0 all pins output low  */
            GPIO_OD_GP14_SD_BYTE_VAL &= ~SD0_PIN;
			 
            /*  Config SD0 to GPO   */
            GPIO_OC_GP14_SD_BYTE_VAL |= SD0_PIN;
		...
		if (MMC0_DRIVER_VERSION == MMC_DRV_3445) {
           ...
           /*Disable Clock & CMD Pull enable*/
			GPIO_PULL_EN_GP14_SD_BYTE_VAL &= ~(GPIO_SD0_Clock | GPIO_SD0_Command);
			
			/*Set CD ,WP ,DATA pin pull up*/
            GPIO_PULL_CTRL_GP3_SD0CD_BYTE_VAL |= GPIO_SD0_CD;
			GPIO_PULL_CTRL_GP14_SD_BYTE_VAL |= (GPIO_SD0_Data | GPIO_SD0_WriteProtect);
         
			
            
			/*Enable CD ,WP ,DATA  internal pull*/
			GPIO_PULL_EN_GP3_SD0CD_BYTE_VAL |= GPIO_SD0_CD;
			GPIO_PULL_EN_GP14_SD_BYTE_VAL |= (GPIO_SD0_Data | GPIO_SD0_WriteProtect);
    ...
    if (ios->clock == mmc->f_min) {
 		host->current_clock = auto_pll_divisor(DEV_SDMMC0, SET_DIV, 1, 390*2)/2;
        DBG("[%s]ATSMB Host 400KHz\n", __func__);            
    }
	else if (ios->clock >= 50000000) {
		host->current_clock = auto_pll_divisor(DEV_SDMMC0, SET_DIV, 2, 45*2)/2;
        DBG("[%s]ATSMB Host 50MHz\n", __func__);
	} else if ((ios->clock >= 25000000) && (ios->clock < 50000000)) {
        host->current_clock = auto_pll_divisor(DEV_SDMMC0, SET_DIV, 2, 24*2)/2;
        DBG("[%s]ATSMB Host 25MHz\n", __func__);
	} else if ((ios->clock >= 20000000) && (ios->clock < 25000000)) {
        host->current_clock = auto_pll_divisor(DEV_SDMMC0, SET_DIV, 2, 20*2)/2;
