...
static inline void wmt_spi_clock_enable(void)
{
	...
	if (g_use_ss1) {
		...
		GPIO_PULL_EN_GP31_PWM_BYTE_VAL |= BIT0;
		GPIO_PULL_CTRL_GP31_PWM_BYTE_VAL |= BIT0;
	} else
		GPIO_CTRL_GP31_PWM_BYTE_VAL |= BIT0;
	if (g_use_ss2) {
		GPIO_CTRL_GP31_PWM_BYTE_VAL &= ~BIT1;
		GPIO_PULL_EN_GP31_PWM_BYTE_VAL |= BIT1;
		GPIO_PULL_CTRL_GP31_PWM_BYTE_VAL |= BIT1;
	} else
		GPIO_CTRL_GP31_PWM_BYTE_VAL |= BIT1;
	if (g_use_ss3) {
		GPIO_CTRL_GP31_PWM_BYTE_VAL &= ~BIT2;
		GPIO_PULL_EN_GP31_PWM_BYTE_VAL |= BIT2;
		GPIO_PULL_CTRL_GP31_PWM_BYTE_VAL |= BIT2;
	} else
		GPIO_CTRL_GP31_PWM_BYTE_VAL |= BIT2;
